...
export default function ManagementComplaintsPage() {
  ...
  const fetchComplaints = async () => {
    setLoading(true);
    try {
      const data = await complaintsService.list(filter !== "ALL" ? { status: filter } : undefined);
      setComplaints(data);
    } catch (err) {
        ...
        <div className="p-5 border-b border-surface-200 dark:border-surface-700">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-surface-500 dark:text-surface-400 text-xs mt-0.5">Manage student complaints</p>
            </div>
          </div>
          <div className="flex bg-surface-100 dark:bg-surface-800 p-1 rounded-xl">
            {["ALL", "Pending", "Resolved"].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`flex-1 text-xs font-semibold py-2 rounded-lg transition-all ${filter === f ? "bg-white dark:bg-surface-600 text-surface-900 dark:text-white shadow-sm" : "text-surface-500 dark:text-surface-400 hover:text-surface-700 dark:hover:text-surface-300"}`}
              >
                {f}
              </button>
            ))}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {loading ? (
             <div className="animate-pulse space-y-3">
               {[1, 2, 3].map((i) => (
                 <div key={i} className="h-20 bg-surface-100 dark:bg-surface-800 rounded-2xl"></div>
               ))}
             </div>
          ) : complaints.length === 0 ? (
               ...
               {complaints.map((comp) => (
                 <StaggerItem key={comp.id}>
                   <motion.div
                     ...
                     className={`cursor-pointer p-4 rounded-2xl border transition-all ${selectedComplaint?.id === comp.id ? "bg-primary-50 dark:bg-primary-500/10 border-primary-500" : "bg-white dark:bg-surface-800 border-surface-200 dark:border-surface-700 hover:border-primary-500/50"}`}
                   >
                     <div className="flex justify-between items-start">
                       <h3 className="text-sm font-semibold text-surface-900 dark:text-white line-clamp-1">{comp.title}</h3>
                       {comp.status === "Pending" ? (
                         <div className="flex items-center gap-1 text-amber-600 dark:text-amber-500 text-xs font-medium"><Clock className="w-3.5 h-3.5" /> Pending</div>
                       ) : (
                         <div className="flex items-center gap-1 text-emerald-600 dark:text-emerald-500 text-xs font-medium"><CheckCircle2 className="w-3.5 h-3.5" /> Resolved</div>
                       )}
                     </div>
                     <p className="text-xs text-surface-500 dark:text-surface-400 mt-1 line-clamp-1">{comp.user_name} • {new Date(comp.created_at).toLocaleDateString()}</p>
                   </motion.div>
                 </StaggerItem>
               ))}
             </StaggerContainer>
            ...
            >
              <div>
                <div className="flex items-center gap-3 text-surface-500 dark:text-surface-400 text-sm mb-2">
                  <span className="font-semibold text-surface-900 dark:text-white">{selectedComplaint.user_name}</span>
                  • {selectedComplaint.user_email} • {new Date(selectedComplaint.created_at).toLocaleString()}
                </div>
                <h2 className="text-2xl font-bold text-surface-900 dark:text-white mb-4">{selectedComplaint.title}</h2>
              ...
              {selectedComplaint.status === "Pending" ? (
                <div className="pt-6 border-t border-surface-200 dark:border-surface-800">
                  <h3 className="text-lg font-semibold text-surface-900 dark:text-white mb-3">Provide Solution</h3>
                  <textarea
                    className="w-full h-32 bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all resize-none mb-4"
                ...
                <div className="pt-6 border-t border-surface-200 dark:border-surface-800">
                  ...
                  <p className="text-xs text-surface-500 mt-3 text-right">Resolved at: {new Date(selectedComplaint.updated_at).toLocaleString()}</p>
                </div>
              )}
            </motion.div>
          ) : (
