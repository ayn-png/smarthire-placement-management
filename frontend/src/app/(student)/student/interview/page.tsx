...
import { Brain, Send, RefreshCw, ChevronDown, MessageSquare, Sparkles, Zap, Bot } from "lucide-react";
...
type InterviewType = "TECHNICAL" | "HR" | "MANAGERIAL" | "CASE_STUDY";
...
export default function InterviewPage() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: data.reply, feedback: data.feedback || undefined },
      ]);
    } finally {
      setChatLoading(false);
    }
            ...
            {questions.length > 0 ? (
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-surface-800 dark:text-surface-200 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                          {q.question}
                        </span>
                      </div>
              ...
              <div className="text-center py-16 bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700">
                ...
                <p className="text-surface-500 dark:text-surface-400 text-sm font-medium">
                  Select interview type and click Generate Questions
                </p>
              </div>
            )}
          </div>
      ...
      {activeTab === "mock" && (
        <FadeIn delay={0.15}>
          <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-surface-500 dark:text-surface-400 mt-0.5">
                      Practice real interview conversations with instant feedback
                    </p>
                  </div>
                </div>
                <Button
                  onClick={startMockInterview}
                  variant={messages.length > 0 ? "secondary" : "gradient"}
                >
            ...
            {messages.length > 0 && (
              <div
                className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700 flex flex-col shadow-sm"
                style={{ height: "500px" }}
              >
                {/* Chat messages */}
